...
<script>
  ...
  export default {
    ...
    created() {
      ...
      axios.get('http://localhost:8088/user/getAddress?ID=' + s.id.toString())
        .then((response) => {
    ...
    methods: {
      ...
      editAction() {
        axios.post('http://localhost:8088/user/changeAddress', this.formData)
          .then((response) => {
            console.log(response);
            this.modal = false;
      ...
      del(index) {
        this.$Modal.confirm({
          ...
          content: '你确定删除这个收货地址',
          onOk: () => {
            axios.post('http://localhost:8088/user/delAddress', this.address[index])
              .then((response) => {
                console.log(response);
